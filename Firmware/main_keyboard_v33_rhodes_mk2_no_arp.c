...
/*
 * KeyStep Keyboard V33 - Rhodes MK2-ish Electric Piano Mono Keys Real Matrix PE/PD
 *
...
/* Envolventes 0..4096. V33: electric piano Rhodes MK2-ish, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V33: Rhodes MK2-ish: golpe de tine corto, cuerpo redondo y decay natural, sin sustain infinito. */
#define AMP_ATTACK_STEP       4096U
#define AMP_DECAY_STEP        2U
#define AMP_SUSTAIN_LEVEL     0U
...
#define FILTER_DECAY_STEP     75U
#define FILTER_SUSTAIN_LEVEL  0U
#define FILTER_RELEASE_STEP   140U

#define PORTAMENTO_SHIFT      0U    /* sin glide: feel mas teclado/electric piano */
#define EP_TREMOLO_INC        820000UL   /* tremolo muy suave tipo EP */
...
static uint32_t ep_noise_seed = 0x1234ABCDUL;
...
static void synth_trigger_key(uint8_t key_index, uint8_t retrigger)
{
  ...
  if (retrigger)
  {
    /* Golpe tipo martillo/tine: reinicia el ataque, sin portamento ni sustain artificial. */
    v->inc1 = synth_target_inc;
    v->phase1 = 0U;
    v->phase2 = 0x20000000UL + ((uint32_t)key_index * 0x01010101UL);
    v->phase_sub = 0x60000000UL + ((uint32_t)key_index * 0x00110011UL);
    v->amp_env = 0U;
    v->filter_env = 0U;
    v->filter_state = 0;
    ep_noise_seed ^= ((uint32_t)key_index + 1UL) * 0x45D9F3BUL;
  }
}
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* V33 Rhodes MK2-ish:
   * - cuerpo redondo casi seno fundamental
   * - tine/bell corto, inharmonico y con decay rapido
   * - muy poca FM sostenida para evitar campana DX/metálica
   * - pequeño ruido de martillo solo al ataque
   */
  v->inc2 = v->inc1 * 2U + (v->inc1 / 180U);      /* tine levemente inharmonico */
  v->inc_sub = v->inc1 * 4U + (v->inc1 / 35U);    /* bell/tine alto, decae rapido */

  uint32_t car_phase = v->phase1;
  uint32_t tine_phase = v->phase2;
  uint32_t bell_phase = v->phase_sub;

  int32_t mod1 = sin8_lookup(tine_phase);
  int32_t mod2 = sin8_lookup(bell_phase);

  v->filter_state = (v->filter_state * 15 + mod1) / 16;

  uint32_t fm_env = (uint32_t)v->filter_env; /* 0..4096, cae rápido */

  /* Indice bajo: el color está en el golpe, no en un metal permanente. */
  int64_t phase_offset = ((int64_t)mod1 * (int32_t)(1800U + fm_env * 7U)) +
                         ((int64_t)mod2 * (int32_t)(fm_env * 4U));

  int32_t body = sin8_lookup(car_phase + (uint32_t)phase_offset);
  int32_t tine = sin8_lookup(tine_phase + (uint32_t)(phase_offset / 5));
  int32_t bell = sin8_lookup(bell_phase + (uint32_t)(phase_offset / 9));

  v->phase1 += v->inc1;
  v->phase2 += v->inc2;
  v->phase_sub += v->inc_sub;

  /* Click/tine inicial: poco volumen, solo cuando filter_env está alto. */
  ep_noise_seed ^= ep_noise_seed << 13;
  ep_noise_seed ^= ep_noise_seed >> 17;
  ep_noise_seed ^= ep_noise_seed << 5;
  int32_t hammer = (int32_t)((ep_noise_seed >> 20) & 0x7FFU) - 1024;

  int32_t tine_amt = 360U + (fm_env / 3U);
  int32_t bell_amt = fm_env / 5U;
  int32_t click_amt = fm_env / 9U;

  int32_t x = (body * 3300 + tine * tine_amt + bell * bell_amt + hammer * click_amt) >> 12;

  /* Tremolo/pickup muy leve para que respire sin sonar a efecto exagerado. */
  ep_lfo_phase += EP_TREMOLO_INC;
  int32_t trem = 4096 + (sin8_lookup(ep_lfo_phase) / 22);
  x = (x * trem) >> 12;
