...
/*
 * KeyStep Keyboard V36 - MultiEngine Mono Synth + Octave Real Matrix PE/PD
 *
 ...
 *   DAC2 / PA5 / Mod CV   = indicador/control simple, queda centrado por defecto.
 *
...
/* Envolventes 0..4096. V35: MultiEngine mono, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V35: valores base; cada motor ajusta su comportamiento internamente. */
...
static uint32_t inc_for_key(uint8_t key_index)
{
  ...
  /* V36: cambio de octava desde combo oculto. Limitado para que el DAC no se vuelva puro aliasing. */
  if (octave_shift > 0)
  {
...
static uint8_t handle_mode_combo(uint32_t mask)
{
  ...
  /* V36: octavas desde la misma tecla FUNC. */
