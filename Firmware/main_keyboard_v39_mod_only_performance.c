...
/*
 * KeyStep Keyboard V39 - Mod CV Audio Only Performance
 *
 ...
 *
 * Arquitectura V39 según prueba real del usuario:
 *   La salida física que se escucha como audio es MOD CV.
 *   Pitch CV no importa en esta versión.
 *   Para evitar confusión, el audio se copia a ambos DAC internamente,
 *   pero la salida usable/principal sigue siendo MOD CV.
 *
...
/* Envolventes 0..4096. V39: MultiEngine mono performance, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V39: valores base; cada motor ajusta su comportamiento internamente. */
...
static uint8_t hold_latch = 0U;       /* V39: 0 normal, 1 sostiene la ultima nota */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    Bare_DAC_Set(audio, audio); /* V39: espejo en ambos DAC; usa Mod CV como salida principal */
...
static uint32_t inc_for_key(uint8_t key_index)
{
  ...
  /* V39: cambio de octava desde combo oculto. Limitado para que el DAC no se vuelva puro aliasing. */
  if (octave_shift > 0)
  {
...
static uint8_t handle_mode_combo(uint32_t mask)
{
  ...
  /* V39: octavas desde la misma tecla FUNC. */
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* V39: macros de performance. Brillo controla el indice FM / apertura de filtro. */
