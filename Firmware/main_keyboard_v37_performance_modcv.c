...
/*
 * KeyStep Keyboard V37 - MultiEngine Performance + Octave + Mod CV
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = mono MultiEngine refinado: MicroFreak-ish, Yamaha FM, Moog lead, Acid bass, Digital bell.
 *   DAC2 / PA5 / Mod CV   = salida de control: envelope + LFO lento para mover un sinte externo.
 *
...
/* Envolventes 0..4096. V37: MultiEngine mono performance, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V37: valores base; cada motor ajusta su comportamiento internamente. */
...
static uint16_t ModCV_Render(void);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    Bare_DAC_Set(Poly_RenderSample(), ModCV_Render());
...
static uint32_t inc_for_key(uint8_t key_index)
{
  ...
  /* V37: cambio de octava desde combo oculto. Limitado para que el DAC no se vuelva puro aliasing. */
  if (octave_shift > 0)
  {
...
static uint8_t handle_mode_combo(uint32_t mask)
{
  ...
  /* V37: octavas desde la misma tecla FUNC. */
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* V37: macros de performance. Brillo controla el indice FM / apertura de filtro. */
...
static uint16_t ModCV_Render(void)
{
  /* PA5 / Mod CV: control lento para sinte externo.
   * Centro alrededor de 2.0V aprox, con mezcla de envolvente + LFO.
   * No es audio principal.
   */
  PolyVoice *v = &voices[0];
  int32_t lfo = sin8_lookup(micro_lfo_phase + 0x40000000UL);
  int32_t env = (int32_t)v->filter_env;

  int32_t amount = 360;
  if (tone_brightness == 0U) amount = 220;
  if (tone_brightness == 2U) amount = 560;

  int32_t out = (int32_t)DAC_MID;
  out += (env * amount) / 4096;
  out += lfo / ((vibrato_depth == 2U) ? 4 : 7);

  if (!synth_gate && v->amp_env == 0U)
  {
    out = DAC_MID + (lfo / 12);
  }

  if (out < 100) out = 100;
  if (out > 3990) out = 3990;
  return (uint16_t)out;
}
