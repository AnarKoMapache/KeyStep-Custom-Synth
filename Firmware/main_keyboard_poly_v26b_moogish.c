...
/*
 * KeyStep Keyboard Note V26 - Poly Moogish Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = sintetizador polifonico digital 4 voces estilo Moog-ish.
...
#define MOOG_SAMPLE_RATE_HZ  14000UL
#define MOOG_SAMPLE_PERIOD_US 71U

/* Envolventes 0..4096. Ataque rápido, decay lento y sustain medio: más tipo sinte analógico. */
#define MAX_POLY_VOICES       4U

#define AMP_ATTACK_STEP       120U
#define AMP_DECAY_STEP        4U
#define AMP_SUSTAIN_LEVEL     2600U
#define AMP_RELEASE_STEP      22U

#define FILTER_ATTACK_STEP    210U
#define FILTER_DECAY_STEP     12U
#define FILTER_SUSTAIN_LEVEL  900U
#define FILTER_RELEASE_STEP   28U
...
static void Poly_KeyDown(uint8_t key_index)
{
  ...
  voices[chosen].inc2 = inc1 + (inc1 / 220U);  /* leve desafinación, más estable para acordes */
  voices[chosen].inc_sub = inc1 / 2U;
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  int32_t saw1 = (int32_t)((v->phase1 >> 20) & 0xFFFU) - 2048;
  int32_t saw2 = (int32_t)((v->phase2 >> 20) & 0xFFFU) - 2048;
  int32_t sub  = ((v->phase_sub & 0x80000000UL) != 0U) ? 800 : -800;

  v->phase1 += v->inc1;
  v->phase2 += v->inc2;
  v->phase_sub += v->inc_sub;

  int32_t x = (saw1 * 5 + saw2 * 4 + sub * 2) / 18;
  x = moog_soft_clip((x * 5) / 4);

  uint16_t cutoff = (uint16_t)(6U + (v->filter_env / 52U));
  if (cutoff > 100U) cutoff = 100U;
  if (cutoff < 3U) cutoff = 3U;

  v->filter_state += ((x - v->filter_state) * (int32_t)cutoff) >> 8;

  int32_t y = moog_soft_clip((v->filter_state * 5) / 4);
...
static uint16_t Poly_RenderSample(void)
{
  ...
  /* Compresión/saturación final para acordes. */
