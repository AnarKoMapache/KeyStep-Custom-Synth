...
/*
 * KeyStep Keyboard V34 - MicroFreak-ish Digital Mono Lead Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = mono digital tipo MicroFreak-ish: wavetable, FM suave, wavefolding, sin arpegiador.
...
/* Envolventes 0..4096. V34: digital MicroFreak-ish, wavetable/FM/wavefolding, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V34: menos imitación de instrumento real, más sinte digital con movimiento. */
...
#define PORTAMENTO_SHIFT      5U    /* glide corto, más expresivo tipo lead digital */
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* Portamento corto: más vivo que DX, sin llegar a Moog lento. */
  ...
  /* V34 MicroFreak-ish:
   * - un carrier wavetable morph sine/tri/saw/pulse
   * - FM suave de ratio 2:1
   * - wavefolding controlado por envolvente
   * - movimiento lento interno tipo wavetable scan
   */
  micro_lfo_phase += 115000UL; /* ~0.43 Hz a 16 kHz */

  v->inc2 = v->inc1 * 2U;       /* modulador 2:1 */
  v->inc_sub = v->inc1 * 3U;    /* color digital 3:1 */

  uint32_t car_phase = v->phase1;
  uint32_t mod_phase = v->phase2;
  uint32_t aux_phase = v->phase_sub;

  int32_t lfo = sin8_lookup(micro_lfo_phase);          /* -2047..2047 */
  uint16_t morph = (uint16_t)((uint32_t)(lfo + 2048) & 0x0FFFU); /* 0..4095 aprox */
  uint16_t pulse_width = (uint16_t)(1300U + ((uint32_t)(lfo + 2048) / 3U));

  int32_t mod = sin8_lookup(mod_phase);
  int32_t aux = tri12_lookup(aux_phase);

  uint32_t fm_env = (uint32_t)v->filter_env;
  int32_t fm_index = 35000 + (int32_t)(fm_env * 92U) + (int32_t)((uint32_t)(lfo + 2048) * 16U);
  uint32_t warped_phase = car_phase + (uint32_t)((int64_t)mod * fm_index);

  int32_t sine = sin8_lookup(warped_phase);
  int32_t tri  = tri12_lookup(warped_phase + ((uint32_t)aux << 9));
  int32_t saw  = saw12_lookup(warped_phase + ((uint32_t)mod << 8));
  int32_t pls  = pulse_lookup(warped_phase, pulse_width);
  ...
  if (morph < 1365U)
  {
    /* sine -> triangle */
    uint32_t m = morph;
    x = (int32_t)(((int64_t)sine * (1365 - m) + (int64_t)tri * m) / 1365);
  }
  else if (morph < 2730U)
  {
    /* triangle -> saw */
    uint32_t m = morph - 1365U;
    x = (int32_t)(((int64_t)tri * (1365 - m) + (int64_t)saw * m) / 1365);
  }
  else
  {
    /* saw -> pulse, pero mezclado suave */
    uint32_t m = morph - 2730U;
    x = (int32_t)(((int64_t)saw * (1365 - m) + (int64_t)pls * m) / 1365);
  }

  /* Un poco de carrier agudo FM para el ataque digital, pero sin campana Rhodes. */
  int32_t bright = sin8_lookup((car_phase * 2U) + (uint32_t)((int64_t)aux * (22000 + (int32_t)fm_env * 20)));
  x = (x * 5 + bright) / 6;

  /* Wavefolding: entra más fuerte al ataque y luego se calma. */
  int32_t drive = 4096 + ((int32_t)v->filter_env / 2) + ((lfo + 2048) / 8);
  x = (x * drive) >> 12;
  x = wavefold(x);

  /* Filtro digital simple, compartiendo el estado del voice.
   * Cutoff abre con el ataque y se mueve apenas con LFO.
   */
  int32_t cutoff = 120 + ((int32_t)v->filter_env >> 4) + ((lfo + 2048) >> 6);
  if (cutoff > 720) cutoff = 720;
  if (cutoff < 80) cutoff = 80;
  v->filter_state += ((x - v->filter_state) * cutoff) >> 10;
  x = (x * 2 + v->filter_state * 3) / 5;
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* FM estilo MicroFreak-ish.
   * Un solo carrier principal con moduladores de ratio 2:1 y 5:1.
   * Mucho brillo al ataque, sustain mas redondo y menos portamento.
   */
  v->inc2 = v->inc1 * 2U;              /* modulador principal ratio 2:1 */
  v->inc_sub = v->inc1 * 5U;           /* modulador agudo ratio 5:1 */
  ...
  /* Feedback de operador: mas parecido a timbre FM Yamaha, sin hacer masa. */
  ...
  /* Indice FM: ataque metalico, luego cae a tono mas limpio. */
  int32_t index_main = 90000 + (int32_t)(fm_env * 420U);
  int32_t index_side = 25000 + (int32_t)(fm_env * 115U);
  ...
  /* Segundo carrier suave una octava arriba: da brillo tipo teclado FM sin ensuciar. */
  int32_t carrier2 = sin8_lookup((car_phase << 1) + (uint32_t)(phase_offset / 5));

  v->phase1 += v->inc1;
  v->phase2 += v->inc2;
  v->phase_sub += v->inc_sub;

  int32_t x = (carrier1 * 6 + carrier2) / 7;
