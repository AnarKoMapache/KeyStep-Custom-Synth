...
/*
 * KeyStep Keyboard V40 - Mod CV Velocity Performance
 *
 ...
 *
 * Arquitectura V40 según prueba real del usuario:
 *   La salida física que se escucha como audio es MOD CV.
 *   Pitch CV no importa en esta versión.
 *   La salida usable/principal sigue siendo MOD CV.
 *   V40 agrega lectura de doble contacto A/B para acento/velocity aproximada.
 *
...
/* Envolventes 0..4096. V40: MultiEngine mono performance + velocity/accent, sin arpegiador. */
#define MAX_POLY_VOICES       1U

/* V40: valores base; cada motor ajusta su comportamiento internamente. */
 ...
 * key16 = hold/latch on-off
 * key15 = velocity/accent off
 * key14 = velocity/accent on
 */
...
#define MODE_KEY_HOLD_TOGGLE   16U
#define MODE_KEY_VELOCITY_OFF   15U
#define MODE_KEY_VELOCITY_ON    14U
...
static uint8_t hold_latch = 0U;       /* 0 normal, 1 sostiene la ultima nota */
static uint8_t velocity_mode = 1U;     /* V40: usa doble contacto A/B como acento aproximado */
static uint8_t current_velocity = 96U; /* 45..127 aprox */
...
static uint8_t KeyboardMatrix_ContactMaskForKey(const uint8_t rows[8], uint8_t key);
static uint8_t velocity_from_timer(uint8_t ticks);
static uint32_t VelocityTracker_UpdateAndMakeMask(const uint8_t rows[8]);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    uint16_t audio = Poly_RenderSample();
    Bare_DAC_Set(audio, audio); /* V40: audio usable por Mod CV; espejo interno por compatibilidad */
...
static void beep(uint32_t freq_hz, uint32_t duration_ms)
{
  ...
  {
    Bare_DAC_Set(DAC_HIGH, DAC_HIGH);
    delay_us(half_period_us);
    Bare_DAC_Set(DAC_LOW, DAC_LOW);
    delay_us(half_period_us);
  }
...
/* V40 velocity/accent tracker:
 * stage 0 = tecla libre
 * stage 1 = primer contacto visto, esperando segundo contacto
 * stage 2 = nota validada
 * timer cuenta scans (~3 ms) entre A y B. Menos tiempo = mas fuerte/brillante.
 */
static uint8_t key_stage[32];
static uint8_t key_timer[32];
static uint8_t key_velocity[32];
...
static uint32_t inc_for_key(uint8_t key_index)
{
  ...
  /* V40: cambio de octava desde combo oculto. Limitado para que el DAC no se vuelva puro aliasing. */
  if (octave_shift > 0)
  {
...
static void synth_trigger_key(uint8_t key_index, uint8_t retrigger)
{
  ...
  if (velocity_mode)
  {
    current_velocity = key_velocity[key_index];
    if (current_velocity < 45U) current_velocity = 45U;
    if (current_velocity > 127U) current_velocity = 127U;
  }
  else
  {
    current_velocity = 96U;
  }
...
static uint8_t handle_mode_combo(uint32_t mask)
{
  ...
  /* V40: octavas desde la misma tecla FUNC. */
  ...
  if ((mask & (1UL << MODE_KEY_HOLD_TOGGLE)) != 0U)
  {
    hold_latch ^= 1U;
    if (!hold_latch)
    {
  ...
  if ((mask & (1UL << MODE_KEY_VELOCITY_OFF)) != 0U)
  {
    velocity_mode = 0U;
    current_velocity = 96U;
    return 1U;
  ...
  if ((mask & (1UL << MODE_KEY_VELOCITY_ON)) != 0U)
  {
    velocity_mode = 1U;
    return 1U;
  }
...
static uint8_t velocity_from_timer(uint8_t ticks)
{
  /* Scan aprox cada 3ms. Menos scans entre contactos = toque mas fuerte. */
  if (ticks <= 1U) return 127U;
  if (ticks <= 2U) return 118U;
  if (ticks <= 3U) return 108U;
  if (ticks <= 5U) return 96U;
  if (ticks <= 8U) return 82U;
  return 64U;
}
...
static uint32_t VelocityTracker_UpdateAndMakeMask(const uint8_t rows[8])
{
  ...
  for (uint8_t key = 0U; key < 32U; key++)
  {
    uint8_t cm = KeyboardMatrix_ContactMaskForKey(rows, key);
    ...
    if (key_stage[key] == 0U)
    {
      key_stage[key] = 1U;
      key_timer[key] = 0U;
      key_velocity[key] = 72U;
    }
    else if (key_stage[key] == 1U)
    {
      if (key_timer[key] < 250U)
      {
        key_timer[key]++;
      }

      if (cm == (CONTACT_A | CONTACT_B))
      {
        key_velocity[key] = velocity_from_timer(key_timer[key]);
        key_stage[key] = 2U;
      }
      else if (key_timer[key] > 9U)
      {
        /* Si solo aparece un contacto, igual validamos suave para no perder notas. */
        key_velocity[key] = 58U;
        key_stage[key] = 2U;
      }
    }
...
static void Poly_UpdateKeyboardDebounced(void)
{
  /* V40: escaneo más rápido (~3 ms) para captar el tiempo A/B de velocity. */
  scan_divider++;
  if (scan_divider < 48U)
  {
    return;
  ...
  uint32_t raw = VelocityTracker_UpdateAndMakeMask(rows);
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  /* V40: velocity abre/cierra timbre y cambia nivel. */
  if (velocity_mode)
  {
    env = (env * (70U + (uint32_t)current_velocity)) / 170U;
    if (env > 4096U) env = 4096U;
  }

  /* V40: macros de performance + velocity. Brillo controla el indice FM / apertura de filtro. */
  ...
  if (velocity_mode)
  {
    /* 64 aprox suave, 127 fuerte. Factor moderado para no saturar el DAC. */
    y = (y * (int32_t)(92U + current_velocity)) / 210;
  }
  return y;
}
