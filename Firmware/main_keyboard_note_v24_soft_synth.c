...
/*
 * KeyStep Keyboard Note V24 - Soft Synth Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = tono suave por wavetable mientras se mantiene presionada.
...
#define SOFT_SAMPLE_RATE_HZ  16000UL
#define SOFT_SAMPLE_PERIOD_US 62U
#define WAVE_TABLE_BITS      6U
#define WAVE_TABLE_SIZE      (1U << WAVE_TABLE_BITS)
#define ATTACK_STEP          4U
#define RELEASE_STEP         4U


static const uint16_t soft_wave_table[WAVE_TABLE_SIZE] = {
  2048, 2202, 2352, 2494, 2625, 2743, 2845, 2930,
  2997, 3047, 3079, 3095, 3098, 3088, 3069, 3042,
  3008, 2970, 2928, 2884, 2838, 2790, 2739, 2686,
  2630, 2570, 2506, 2438, 2365, 2290, 2211, 2130,
  2048, 1966, 1885, 1806, 1731, 1658, 1590, 1526,
  1466, 1410, 1357, 1306, 1258, 1212, 1168, 1126,
  1088, 1054, 1027, 1008, 998, 1001, 1017, 1049,
  1099, 1166, 1251, 1353, 1471, 1602, 1744, 1894
};
...
static void play_note_while_held(uint8_t key_index)
{
  ...
  uint32_t phase = 0U;
  uint32_t phase_inc = (uint32_t)(((uint64_t)freq_hz << 32) / SOFT_SAMPLE_RATE_HZ);
  uint16_t env = 0U; /* 0..256 */
  uint8_t check_counter = 0U;

  while (1)
  {
    uint8_t idx = (uint8_t)(phase >> (32U - WAVE_TABLE_BITS));
    int32_t centered = (int32_t)soft_wave_table[idx] - (int32_t)DAC_MID;
    uint16_t out = (uint16_t)((int32_t)DAC_MID + ((centered * (int32_t)env) / 256));
    ...
    if (env < 256U)
    {
      env = (uint16_t)((env + ATTACK_STEP) > 256U ? 256U : (env + ATTACK_STEP));
    }

    delay_us(SOFT_SAMPLE_PERIOD_US);

    /* Revisa la matriz unas 300 veces por segundo, sin cortar el audio tan seguido. */
    check_counter++;
    if (check_counter >= 48U)
    {
      uint8_t rows[8];
  ...
  /* Release suave para evitar click al soltar. */
  while (env > 0U)
  {
    uint8_t idx = (uint8_t)(phase >> (32U - WAVE_TABLE_BITS));
    int32_t centered = (int32_t)soft_wave_table[idx] - (int32_t)DAC_MID;
