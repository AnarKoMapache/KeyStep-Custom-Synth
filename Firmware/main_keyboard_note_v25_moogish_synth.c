...
/*
 * KeyStep Keyboard Note V25 - Moogish Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = tono estilo Moogish: dos sierras desafinadas + sub + filtro/envolvente.
...
#define AMP_ATTACK_STEP       96U
#define AMP_DECAY_STEP        3U
#define AMP_SUSTAIN_LEVEL     2500U
#define AMP_RELEASE_STEP      18U

#define FILTER_ATTACK_STEP    180U
#define FILTER_DECAY_STEP     10U
#define FILTER_SUSTAIN_LEVEL  850U
#define FILTER_RELEASE_STEP   22U
...
static uint16_t moog_render_sample(uint32_t *phase1, uint32_t *phase2, uint32_t *phase_sub,
                                   uint32_t inc1, uint32_t inc2, uint32_t inc_sub,
                                   uint16_t amp_env, uint16_t filter_env, int32_t *filter_state);
...
static uint32_t note_freq_for_key(uint8_t key_index)
{
   ...
   *
   * Si queda invertido físicamente, basta invertir esta tabla en V25.
   */
...
static int32_t moog_soft_clip(int32_t x)
{
  /* Saturación simple: evita que el DAC pegue fuerte y suena menos digital. */
  if (x > 1700)  x = 1700 + (x - 1700) / 4;
  if (x < -1700) x = -1700 + (x + 1700) / 4;
  if (x > 2047)  x = 2047;
  if (x < -2047) x = -2047;
  return x;
...
static uint16_t moog_render_sample(uint32_t *phase1, uint32_t *phase2, uint32_t *phase_sub,
                                   ...
                                   uint16_t amp_env, uint16_t filter_env, int32_t *filter_state)
{
  /* Dos sierras + sub cuadrada. Todo centrado en 0. */
  int32_t saw1 = (int32_t)((*phase1 >> 20) & 0xFFFU) - 2048;
  int32_t saw2 = (int32_t)((*phase2 >> 20) & 0xFFFU) - 2048;
  int32_t sub  = ((*phase_sub & 0x80000000UL) != 0U) ? 950 : -950;

  *phase1 += inc1;
  *phase2 += inc2;
  *phase_sub += inc_sub;

  /* Mezcla gruesa tipo monosinte. */
  int32_t x = (saw1 * 5 + saw2 * 4 + sub * 2) / 11;
  x = moog_soft_clip((x * 3) / 2);

  /* Filtro pasa-bajos de 1 polo con envolvente. No es un ladder real, pero se acerca más. */
  uint16_t cutoff = (uint16_t)(7U + (filter_env / 42U)); /* aprox 7..104 */
  if (cutoff > 120U) cutoff = 120U;
  if (cutoff < 4U) cutoff = 4U;

  *filter_state += ((x - *filter_state) * (int32_t)cutoff) >> 8;

  int32_t y = moog_soft_clip((*filter_state * 5) / 4);
  y = (y * (int32_t)amp_env) >> 12;
  y = moog_soft_clip(y);

  int32_t out = (int32_t)DAC_MID + y;
  if (out < 80) out = 80;
  if (out > 4015) out = 4015;
  return (uint16_t)out;
}
...
static void play_note_while_held(uint8_t key_index)
{
  ...
  uint32_t phase1 = 0U;
  uint32_t phase2 = 0x12345678UL;
  uint32_t phase_sub = 0U;

  uint32_t inc1 = (uint32_t)(((uint64_t)freq_hz << 32) / MOOG_SAMPLE_RATE_HZ);
  /* Un poco desafinado para grosor, no tanto para que no parezca desafinado. */
  uint32_t inc2 = inc1 + (inc1 / 180U);
  uint32_t inc_sub = inc1 / 2U;

  uint16_t amp_env = 0U;
  uint16_t filter_env = 0U;
  int32_t filter_state = 0;
  uint8_t check_counter = 0U;

  while (1)
  {
    uint16_t out = moog_render_sample(&phase1, &phase2, &phase_sub,
                                      inc1, inc2, inc_sub,
                                      amp_env, filter_env, &filter_state);
    Bare_DAC_Set(out, DAC_MID);

    if (amp_env < 4096U)
    {
      amp_env = (uint16_t)(((uint32_t)amp_env + AMP_ATTACK_STEP > 4096U) ? 4096U : amp_env + AMP_ATTACK_STEP);
    }
    else if (amp_env > AMP_SUSTAIN_LEVEL)
    {
      amp_env = (uint16_t)((amp_env > AMP_DECAY_STEP + AMP_SUSTAIN_LEVEL) ? amp_env - AMP_DECAY_STEP : AMP_SUSTAIN_LEVEL);
    }

    if (filter_env < 4096U)
    {
      filter_env = (uint16_t)(((uint32_t)filter_env + FILTER_ATTACK_STEP > 4096U) ? 4096U : filter_env + FILTER_ATTACK_STEP);
    }
    else if (filter_env > FILTER_SUSTAIN_LEVEL)
    {
      filter_env = (uint16_t)((filter_env > FILTER_DECAY_STEP + FILTER_SUSTAIN_LEVEL) ? filter_env - FILTER_DECAY_STEP : FILTER_SUSTAIN_LEVEL);
    }

    delay_us(MOOG_SAMPLE_PERIOD_US);

    check_counter++;
    if (check_counter >= 40U)
    {
      uint8_t rows[8];
  ...
  /* Release tipo sinte: no corta seco. */
  while (amp_env > 0U)
  {
    uint16_t out = moog_render_sample(&phase1, &phase2, &phase_sub,
                                      inc1, inc2, inc_sub,
    ...
    amp_env = (uint16_t)((amp_env > AMP_RELEASE_STEP) ? amp_env - AMP_RELEASE_STEP : 0U);
    filter_env = (uint16_t)((filter_env > FILTER_RELEASE_STEP) ? filter_env - FILTER_RELEASE_STEP : 0U);
