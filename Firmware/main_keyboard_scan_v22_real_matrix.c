...
/*
 * KeyStep Keyboard Scan V22 - Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = tono audible por tecla.
 *   DAC2 / PA5 / Mod CV   = marcador A/B para medir, no necesario escuchar.
 *
...
static void report_key(uint8_t key_index, uint8_t contact_mask);
...
int main(void)
{
  ...
  while (1)
  {
    ...
    if (ev.valid)
    {
      report_key(ev.key_index, ev.contact_mask);
      wait_release();
      pause_ms(120);
    }
  }
...
{
  /*
   * key_index 0..31 viene del firmware oficial:
   * nota aproximada = 72 - key_index.
   * Para test usamos una frecuencia audible invertida:
   * key 31 = grave, key 0 = aguda.
   */
  uint32_t freq = 180U + (uint32_t)(31U - key_index) * 22U;
  uint8_t contact_code = 0U;

  if ((contact_mask & CONTACT_A) != 0U) contact_code += 1U;
  if ((contact_mask & CONTACT_B) != 0U) contact_code += 2U;

  /* Marcador en DAC2: A bajo, B alto, A+B muy alto. */
  if (contact_code == 1U) Bare_DAC_Set(DAC_MID, 1100U);
  else if (contact_code == 2U) Bare_DAC_Set(DAC_MID, 3000U);
  else Bare_DAC_Set(DAC_MID, 3800U);

  /* Inicio de evento. */
  beep(1500, 80);
  pause_ms(80);

  /* Contacto: A=1 beep, B=2 beeps, A+B=3 beeps. */
  beep_count(contact_code, 760);

  /* Key index codificado en dos grupos: decenas y unidades.
   * Para key 0, decenas=0 y unidades=1, asi no queda silencio.
   */
  uint8_t tens = key_index / 10U;
  uint8_t ones = key_index % 10U;

  beep_count(tens, 520);
  beep_count((uint8_t)(ones + 1U), 980);

  /* Tono principal segun tecla. */
  beep(freq, 320);
  pause_ms(180);
