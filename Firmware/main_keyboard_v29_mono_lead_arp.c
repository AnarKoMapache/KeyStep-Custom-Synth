...
/*
 * KeyStep Keyboard Note V29 - Mono Lead Moog + Arpeggiator Real Matrix PE/PD
 *
 ...
 *
 * Audio de prueba:
 *   DAC1 / PA4 / Pitch CV = Mono Lead Moog-ish con portamento + arpegiador interno.
...
/* V29: mono lead. Ataque rapido, sustain firme y release suave. */
#define AMP_ATTACK_STEP       330U
#define AMP_DECAY_STEP        12U
#define AMP_SUSTAIN_LEVEL     3200U
#define AMP_RELEASE_STEP      38U

/* Filtro con golpe mas Moog-ish: abre rapido y cae a sustain. */
#define FILTER_ATTACK_STEP    420U
#define FILTER_DECAY_STEP     42U
#define FILTER_SUSTAIN_LEVEL  850U
#define FILTER_RELEASE_STEP   58U

#define PORTAMENTO_SHIFT      10U   /* mayor = glide mas lento */
#define ARP_STEP_SAMPLES      1900U /* aprox 120 ms a 16 kHz */
...
static uint32_t inc_for_key(uint8_t key_index)
{
  ...
  return (uint32_t)(((uint64_t)freq_hz << 32) / MOOG_SAMPLE_RATE_HZ);
}
...
static int32_t Poly_RenderVoice(PolyVoice *v)
{
  ...
  v->inc2 = v->inc1 + (v->inc1 / 305U); /* desafinacion muy leve */
  v->inc_sub = v->inc1 / 2U;

  uint32_t p1 = (v->phase1 >> 20) & 0xFFFU;
  uint32_t p2 = (v->phase2 >> 20) & 0xFFFU;

  int32_t saw1 = (int32_t)p1 - 2048;
  int32_t saw2 = (int32_t)p2 - 2048;
  int32_t pulse = ((v->phase1 >> 31) != 0U) ? 1250 : -1250;
  int32_t sub = ((v->phase_sub >> 31) != 0U) ? 520 : -520;

  v->phase1 += v->inc1;
  v->phase2 += v->inc2;
  v->phase_sub += v->inc_sub;

  /* Oscilador mono: sierra principal + pulso + sub chico. */
  int32_t x = (saw1 * 5 + saw2 * 2 + pulse * 3 + sub) / 11;

  /* Filtro 2 polos simple con feedback tipo resonancia suave. */
  uint8_t key = (synth_current_key <= 31U) ? synth_current_key : 16U;
  uint16_t keytrack = (uint16_t)((31U - key) * 2U);
  uint16_t cutoff = (uint16_t)(20U + keytrack + (v->filter_env / 34U));
  if (cutoff > 190U) cutoff = 190U;
  if (cutoff < 12U) cutoff = 12U;

  int32_t feedback = v->filter_state / 5;
  int32_t input = x - feedback;

  /* Usamos phase2 como storage? No: agregamos segunda etapa con variable local persistente no disponible.
   * Para mantener struct simple, usamos filter_state como filtro principal y mezclamos señal seca filtrada.
   */
  v->filter_state += ((input - v->filter_state) * (int32_t)cutoff) >> 8;

  int32_t y = (v->filter_state * 3 + x) / 4;
